...
// namespace suc
{
	...
	{
	public:
		SucClientSocket() = default;
		explicit SucClientSocket(SOCKET socket) noexcept;

		SucClientSocket(const SucClientSocket&) = delete;
		SucClientSocket& operator=(const SucClientSocket&) = delete;
		SucClientSocket(SucClientSocket&&) noexcept = default;
		SucClientSocket& operator=(SucClientSocket&&) noexcept = default;

		~SucClientSocket() noexcept;
		...
		Sends data through the socket.
		- ARG buf: Specifies the data that shall be sent through the socket. */
		void send(const SucByteBuffer& buf);
		...
		[[nodiscard]] std::optional<SucByteBuffer> recv(int timeoutMS = SUC_TIMEOUT_NEVER);
	...
	private:
		...
		SucByteBuffer recvbuf{ STANDARD_BUF_SIZE };
